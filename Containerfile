...
RUN --mount=type=cache,dst=/var/cache \
    ...
    /ctx/ghcurl "$(/ctx/ghcurl https://api.github.com/repos/ublue-os/yafti-gtk/releases/latest | jq -r '.assets[] | select(.name == "yafti-gtk.flatpak") | .browser_download_url')" -Lo /usr/share/ublue-os/bazzite/yafti-gtk.flatpak && \
