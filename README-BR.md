...
- [Sobre \& Recursos](#sobre--recursos)
  ...
  - [Steam Deck/PCs Centros de Mídia (HTPCs)](#steam-deckpcs-centros-de-mídia-htpcs)
    - [Portáteis Alternativos](#portáteis-handhelds)
  - [GNOME](#gnome)
...
- [Porquê](#porquê)
- [Imagens](#imagens)
- [Documentação \& Newsletters](#documentation--newsletters)
- [Pacotes Customizados](#custom-packages)
...
[Entre no nosso site](https://bazzite.gg/) para uma explicação do Bazzite mais amigável a iniciantes. Este README é uma explicação aprofundada.
...
O Bazzite é feito a partir do [ublue-os/main](https://github.com/ublue-os/main) e [ublue-os/nvidia](https://github.com/ublue-os/nvidia) utilizando tecnologia [Fedora](https://fedoraproject.org/), o que significa que suporte expandido de hardware e outros drivers estão pré-inclusos. Adicionalmente, o Bazzite inclui os seguintes recursos:
...
- Suporte completo aos run-times ROCM OpenCL/HIP da AMD.
...
- Inclui temas GTK3/4 semelhantes ao Vapor e VGUI2 do SteamOS. Instale o [Gradience](https://flathub.org/apps/com.github.GradienceTeam.Gradience) pra utilizá-los.
...
- [Distrobox](https://github.com/89luca89/distrobox) pré-instalado com atualizações automáticas para containers criados pelo usuário.
- [Ptyxis Terminal](https://gitlab.gnome.org/chergert/ptyxis) é utilizado como terminal padrão em todas as imagens. Esse terminal é feito especialmente para o workflow em containers que você deve usar no Bazzite. Se quiser restaurar o terminal padrão, rode `ujust restore-original-terminal`
...
- [Input Remapper](https://github.com/sezanzeb/input-remapper) pré-instalado e habilitado. <sub><sup>(Disponível mas desabilitado por padrão na versão Deck, ative rodando `ujust restore-input-remapper`)</sup></sub>
- O Bazzite Portal provém um jeito fácil de instalar vários aplicativos e ajustes, incluindo a instalação do [LACT](https://github.com/ilya-zlobintsev/LACT) e do [GreenWithEnvy](https://gitlab.com/leinardi/gwe).
- [Waydroid](https://waydro.id/) pré-instalado para rodar aplicativos Android. Leia esse [guia rápido](https://universal-blue.discourse.group/docs?topic=32) para configurá-lo.
...
Imagem genérica disponível como `bazzite`, apropriada para computadores desktop.

- Atualiza automaticamente o sistema, Flatpaks, e todos os containers Distrobox - por meio do [ublue-update](https://github.com/ublue-os/ublue-update) e [topgrade](https://github.com/topgrade-rs/topgrade).
...
(Nota: A solução Open Source Mesa para GPUs NVIDIA, a NVK ainda é propensa a erros quando esta orientação é redigida, pra qualquer problema pertinente à NVK [por favor mande um relatório ao projeto Mesa]([url](https://docs.mesa3d.org/bugs.html)), não ao Ublue/Bazzite)

```bash
rpm-ostree rebase ostree-unverified-registry:ghcr.io/ublue-os/bazzite:stable
```

ou para dispositivos com GPUs Nvidia com os **Drivers Proprietários da NVIDIA**:

```bash
rpm-ostree rebase ostree-unverified-registry:ghcr.io/ublue-os/bazzite-nvidia:stable
```

**Usuários com o Secure Boot ativado:** Sigam nossa [documentação pra secure boot](#secure-boot) antes do rebase.

### Steam Deck/PCs Centros de Mídia (HTPCs)
> [!IMPORTANTE]  
Dispositivos que NÃO SÃO o Steam Deck também podem usar as imagens `bazzite-deck`, mas precisam ter uma GPU recente da AMD. GPUs Arc da Intel também são confirmadas como funcionando.
...
- Vem com patches do [SteamOS BTRFS](https://gitlab.com/popsulfr/steamos-btrfs) para suporte completo a BTRFS no cartão SD por padrão.
- Inclui uma versão do [SDGyroDSU](https://github.com/kmicki/SteamDeckGyroDSU), habilitada por padrão.
- Opção pra instalar o [Decky Loader](https://github.com/SteamDeckHomebrew/decky-loader), [EmuDeck](https://www.emudeck.com/), [RetroDECK](https://retrodeck.net/), e [ProtonUp-Qt](https://davidotek.github.io/protonup-qt/), entre outros vários pacotes úteis na instalação.
- Um sistema de atualização customizado permite que o sistema, Flatpaks e imagens Distrobox sejam atualizadas diretamente pela interface do Game mode via [ublue-update](https://github.com/ublue-os/ublue-update) e [topgrade](https://github.com/topgrade-rs/topgrade).
- Suporte nativo a dual-boot com Windows já que a instalação de GRUB do Fedora é mantida intacta.
...
- [Discover Overlay](https://github.com/trigg/Discover) pra Discord pré-instalado e iniciado automaticamente tanto no Game mode quanto no Desktop se o Discord estiver instalado. [Leia a documentação oficial aqui](https://trigg.github.io/Discover/bazzite).
...
- Recursos para usuários avançados desabilitados por padrão, incluindo:
    ...
    - Um serviço para limitar o nível máximo de carga da bateria, vide `batterylimit.service` e `/etc/default/batterylimit`. <sup><sub>(Funciona mesmo se o dispositivo estiver desligado)</sub></sup>
    - Suporte a overclocking de tela. Por exemplo, adicione `GAMESCOPE_OVERRIDE_REFRESH_RATE=40,70` a `/etc/environment`.
    - Modificou seu Steam Deck pra ter 32GB de RAM? Aproveite o dobro da quantia máxima de VRAM, aplicado automaticamente. <sup><sub>(Me empresta esse talento com solda?)</sub></sup>
- Serviços específicos para o hardware do Steam Deck podem ser desabilitados rodando `ujust disable-bios-updates` e `ujust disable-firmware-updates` no terminal. Eles são desabilitados automaticamente em hardware que não é o Deck, e em Decks com telas DeckHD ou mods de 32GB de RAM.
...
Para fazer rebase de uma instalação existente de Fedora Atômico pra esta imagem:
...
Nós também incluímos comandos `ujust` para instalar vários temas pro [CSS Loader](https://docs.deckthemes.com/CSSLoader/Install/#linux-or-steam-deck) que não se encontram na loja do CSS Loader. Esses temas são atualizados automaticamente com o Bazzite se instalados.
```bash
# Instalar o tema ROG Ally pra CSS Loader (https://github.com/semakusut/SBP-ROG-Ally)
ujust install-rog-ally-theme

# Instalar o tema Lenovo Legion Go pra CSS Loader (https://github.com/frazse/SBP-Legion-Go-Theme)
ujust install-legion-go-theme

# Instalar o tema Handheld Controller (https://github.com/victor-borges/handheld-controller-glyphs)
ujust install-hhd-controller-glyph-theme

# Instalar o tema de glifos PS5-to-Xbox pra hhd & CSS Loader (https://github.com/frazse/PS5-to-Xbox-glyphs)
ujust install-hhd-xbox-glyph-theme
```

### GNOME

Imagens com o ambiente de desktop GNOME estão disponíveis pra ambas as versões desktop & deck. Estas variantes incluem os seguintes recursos adicionais:
...
- Menu customizado no painel para retornar ao game mode, iniciar a Steam, e abrir alguns utilitários úteis.
...
Pra fazer rebase de um sistema ostree existente para uma versão com Ambiente de Desktop e os **Drivers Proprietários da NVIDIA**:
...
**Usuários com Secure Boot ativado:** Sigam nossa [documentação pra secure boot](#secure-boot) antes do rebase.
...
- Faça rollback pra qualquer versão do Bazzite dos últimos 90 dias.

#### Recursos do Fedora Linux (Kinoite & Silverblue)

- Base bastante sólida e estável.
- Pacotes de sistema relativamente atualizados.
...
- A capacidade de mudar a base do sistema entre imagens diferentes de Fedora Atômico, se desejado, sem perder dados de usuário.
...
Apesar desse projeto também ser baseado em imagens, você pode instalar qualquer pacote Fedora direto do terminal. Estes pacotes vão persistir entre atualizações <sub><sup>(Então vá em frente e instale aquele software de VPN desconhecido que você gastou meia-hora tentando fazer funcionar no SteamOS)</sup></sub>. Inclusive, o Bazzite é atualizado várias vezes por semana com os pacotes dos repositórios Fedora, provendo a melhor peformance possível e os últimos recursos - tudo numa base estável.

O Bazzite inclui a versão mais recente do kernel Linux e vem com SELinux habilitado por padrão com suporte total a secure boot <sub><sup>(Rode `ujust enroll-secure-boot-key` e insira a senha `ublue-os` se requisitada para adicionar a nossa chave)</sup></sub> e encriptação de disco, fazendo dele uma solução razoável pra computação em geral. <sup><sub>(Sim, você pode usar sua impressora com o Bazzite)</sub></sup>

Leia o [FAQ](https://universal-blue.discourse.group/docs?topic=33) pra mais detalhes no que faz o Bazzite ser diferente de outros sistemas operacionais Linux.
...
- [Instalando e Gerenciando Aplicativos](https://universal-blue.discourse.group/docs?topic=35)
...
Pacotes vindos do SteamOS e ChimeraOS, entre outros usados pelo Bazzite, são construídos no Copr em [bazzite](https://copr.fedorainfracloud.org/coprs/kylegospo/bazzite/) e [bazzite-multilib](https://copr.fedorainfracloud.org/coprs/kylegospo/bazzite-multilib/).
...
Estas imagens são assinadas com o [cosign](https://docs.sigstore.dev/cosign/overview/) da sigstore. Você pode verificar essa assinatura baixando a chave `cosign.pub` deste repositório ao rodar o seguinte comando:
...
> [!AVISO]  
> **Usuários do Steam Deck: O Steam Deck não habilita o secure boot e não vem com quaisquer chaves inclusas por padrão. Não ative se não tiver certeza absoluta que sabe o que está fazendo.**

Secure boot é suportado com a nossa chave própria. A chave pública se encontra [aqui](https://github.com/ublue-os/bazzite/blob/main/secure_boot.der) na raiz deste repositório. Se você quiser adicionar nossa chave antes da instalação ou rebase, baixe a chave e rode o seguinte:
...
Pra usuários que já estão numa imagem Universal Blue, é só rodar `ujust enroll-secure-boot-key`.
...
- [SuperRiderTH](https://github.com/SuperRiderTH) - Por criar nosso vídeo de inicialização pro Steam game mode.
- [evlaV](https://gitlab.com/evlaV) - Por disponibilizar o código da Valve e por ser [essa pessoa](https://xkcd.com/2347/).
- [ChimeraOS](https://chimeraos.org/) - Pelo gamescope-session e pela assistência significativa ao longo do percurso.
...
- [sentry](https://copr.fedorainfracloud.org/coprs/sentry/) - Por ajudar com patches de kernel necessários e por criar o [repositório kernel-fsync](https://copr.fedorainfracloud.org/coprs/sentry/kernel-fsync/) que usamos agora.
...
Discuta e crie guias de usuário lá nos [Fóruns Discourse do Universal Blue](https://universal-blue.discourse.group/c/bazzite/5).
